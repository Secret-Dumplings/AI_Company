...
此示例展示：
1. 如何用 tangyuanAI.AnthropicAgent 走 Anthropic Messages API
2. 注册工具 + 让 Agent 自动调用
...
# 2. 注册 Anthropic 协议 Agent（v0.3.0+ 模板池写法）
@tangyuanAI.template_agent(
    "weather_agent",
    uuid=uuid.uuid4().hex,
    description="天气查询与温度换算助手，调用 get_weather / celsius_to_fahrenheit",
)
class WeatherAgent(tangyuanAI.AnthropicAgent):
    """
...
if __name__ == "__main__":
    activate_template("weather_agent")

    if not os.getenv("ANTHROPIC_API_KEY"):
        print("未设置 ANTHROPIC_API_KEY，跳过实际对话；只演示装饰器和收集器。")
        # 即便不联网，也能 inspect 已收集到的工具 schema
        tool_registry = tangyuanAI.tool_registry
        for s in tool_registry.collect_builtin_tools(tangyuanAI.agent_list["weather_agent"]):
            fn = s["function"]
