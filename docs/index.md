...
tangyuanAI 支持两种工具调用方式：**Function Calling** 和 **XML**。
...
```python
# 无参数工具
@tangyuanAI.tool_registry.register_tool(name="get_time")
def get_time(**kwargs):
    """获取当前时间"""
    from datetime import datetime
    return datetime.now().strftime("%H:%M:%S")
...
# 单参数工具
@tangyuanAI.tool_registry.register_tool(name="search")
...
@tangyuanAI.register_agent("unique_uuid", "agent_name")
...
```python
@tangyuanAI.register_agent("uuid", "fc_agent")
class FCAgent(tangyuanAI.BaseAgent):
...
```python
@tangyuanAI.register_agent("uuid", "custom_agent")
...
```python
from tangyuanAI.mcp_bridge import _global_session_pool, start_health_check
...
```python
from tangyuanAI.logging_config import setup_logging, get_logger
