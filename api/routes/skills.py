...
async def list_skills() -> SkillListResponse:
    """列出全部已注册的 Skill"""
    raw = tangyuanAI.skill_registry.list_skills() or []
...
async def search_skills(q: str = "") -> SkillListResponse:
    ...
    raw = tangyuanAI.skill_registry.search_skills(q) or []
    items = [_skill_to_info(s) for s in raw]
    return SkillListResponse(skills=items, total=len(items))
...
async def get_skill(name: str) -> dict:
    skill = tangyuanAI.skill_registry.get_skill(name)
    if skill is None:
...
async def scan_skills(req: SkillScanRequest) -> SkillListResponse:
    ...
    tangyuanAI.skill_registry.scan_and_register(paths, auto_watch=False)
    raw = tangyuanAI.skill_registry.list_skills() or []
    items = [_skill_to_info(s) for s in raw]
    return SkillListResponse(skills=items, total=len(items))
...
async def register_skill(req: SkillRegisterRequest) -> SkillListResponse:
    ...
    skill = tangyuanAI.skill_registry.register_skill(path)
    if skill is None:
...
async def unregister_skill(name: str) -> dict:
    """取消注册 Skill"""
    tangyuanAI.skill_registry.unregister_skill(name)
...
async def register_skill_as_tool(name: str) -> dict:
    ...
    try:
        tangyuanAI.register_skill_as_tool(name)
...
async def unregister_skill_as_tool(name: str) -> dict:
    """取消 Skill-as-tool"""
    try:
        tangyuanAI.unregister_skill_from_tool(name)
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e)) from e
    return {"status": "ok", "name": name}